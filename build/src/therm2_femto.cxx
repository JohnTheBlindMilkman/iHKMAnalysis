...
// ##############################################################
    ...
    btmax = 1.0;
        ...
        // maybe turn this into enum or sth? database? or just F. this and let used decide kt cuts in .ini file? - JJ
    switch(nbin) {
    case 0:
        ktmin = 0.15;
        ktmax = 0.25;
...
// ##############################################################
    ...
    PRINT_DEBUG_1("Particle entries: "); //idk how to redo this as of now, leaving for later - JJ
...
// ##############################################################  
    ...
    ofile->Close(); // ...and should be closed - JJ
...
{
    twospin = 0;
    if (pairtype == 0) // this could be a switch to match the rest - JJ
    {
...
void GetFFdouble(dcomplex *ffp, dcomplex *ffm)
{
    ...
    if ((kstar*mRSt*(1+coskr) < 5.0) && (kstar*mRSt*(1-coskr) < 5.0))
        nsteps = 25;
    else if ((kstar*mRSt*(1+coskr) < 10.0) && (kstar*mRSt*(1-coskr) < 10.0))
        nsteps = 45;
    else if ((kstar*mRSt*(1+coskr) < 15.0) && (kstar*mRSt*(1-coskr) < 15.0))
        nsteps = 110;
    else
        nsteps = 150;
    ...
    long double rad = mRSt; // whyyyyyy - JJ
...
double GetQuantumCoulomb()
{
    ...
    return (0.5 * Gamow(fabs(mKStarSigned)) * (modl2(ffplus) + wavesign*sterm.re + wavesign*tterm.re + modl2(ffminus)));
}
...
double GetQuantumCoulombStrong()
{
    ...
    if (twospin == 1) 
    {
        wavesign = 1;
        smult = 2;
        long double singlet = (0.5 * Gamow(kstar) * (2.0 * fgmods * smult + modl2(ffplus) + modl2(ffminus) + wavesign*sterm.re + wavesign*tterm.re + smult * 2 * (fcgefhs + fcgefgs)));
        wavesign = -1;
        smult = 0;
        long double triplet = (0.5 * Gamow(kstar) * (2.0 * fgmodt * smult + modl2(ffplus) + modl2(ffminus) + wavesign*sterm.re + wavesign*tterm.re + smult * 2 * (fcgefht + fcgefgt)));
